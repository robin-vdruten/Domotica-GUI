...
namespace Renderer {
	...
	OpenGLShader::OpenGLShader(const std::string& filepath)
		: m_FilePath(filepath)
	{
		std::string source = Utils::ReadTextFile(filepath);
	...
	{
		for (auto&& [stage, sourcepath] : shaderSources)
		{
			GLuint ShaderHandle = glCreateShader(stage);
			std::string source = sourcepath;

			const GLchar* sourceCStr = (const GLchar*)source.c_str();
			glShaderSource(ShaderHandle, 1, &sourceCStr, 0);
			...
			m_OpenGL.push_back(ShaderHandle);
		}
	}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		for (auto&& stage : m_OpenGL)
		{
			GLuint shaderID = shaderIDs.emplace_back(stage);
