...
namespace Renderer {
	...
	struct MeshInstance
	{
		...
		// Callback to write this instance's vertices into the mesh VB.
		// Signature:
		//   dstVertexPtr: pointer into the mesh vertex buffer
		//   transform:    world transform for this instance
		//   firstVertex:  index of the first vertex for this instance (for index building)
		// Returns: number of vertices written.
		std::function<uint32_t(void* dstVertexPtr, const glm::mat4& transform, uint32_t firstVertex)> BuildVertices;
	};
	...
	struct Renderer2DData
	{
		...
		static const uint32_t MaxMeshVertices = MaxVertices;  // you can choose another cap
		...
		std::array<MeshInstance, 1028> MeshInstances;
	...
	void Renderer2D::Init()
	{
		...
		// Enough space; layout will be whatever the user VA expects.
		s_Data.MeshVertexBuffer = VertexBuffer::Create(Renderer2DData::MaxMeshVertices * sizeof(float) * 8);
		...
		// CPU-side mesh vertex buffer
		s_Data.MeshVertexBufferBase = new uint8_t[Renderer2DData::MaxMeshVertices * sizeof(float) * 8];
		...
		if (s_Data.MeshInstanceCount > 0)
		{
			struct MeshKey
			{
				...
				bool operator==(const MeshKey& other) const {
					return VA == other.VA && ShaderPtr == other.ShaderPtr;
				}
			};
			struct MeshKeyHash
			{
				std::size_t operator()(const MeshKey& key) const {
					return (std::size_t)key.VA ^ ((std::size_t)key.ShaderPtr << 1);
				}
			};

			static std::unordered_map<MeshKey, std::vector<MeshInstance*>, MeshKeyHash> batches;
			static std::vector<uint32_t> batchIndices;

			for (auto& [key, instances] : batches)
				instances.clear();

			for (uint32_t i = 0; i < s_Data.MeshInstanceCount; ++i)
			{
				auto* inst = &s_Data.MeshInstances[i];
				batches[{inst->VertexArray.get(), inst->Shader.get()}].push_back(inst);
			}
			...
			for (auto& [key, instances] : batches)
			{
				...
				for (MeshInstance* inst : instances)
					totalIndexCount += inst->IndexCount;
				batchIndices.reserve(totalIndexCount);
				totalIndexCount = 0;

				const auto& layout = key.VA->GetVertexBuffers()[0]->GetLayout();
				uint32_t vertexSize = layout.GetStride();

				for (MeshInstance* inst : instances)
				{
					uint32_t verticesWritten = inst->BuildVertices(
						s_Data.MeshVertexBufferPtr,
						inst->Transform,
						totalVertexCount
					);
					s_Data.MeshVertexBufferPtr = (uint8_t*)s_Data.MeshVertexBufferPtr + (verticesWritten * vertexSize);

					const uint32_t* localIndices = inst->VertexArray->GetIndexBuffer()->GetIndices();
					uint32_t idxCount = inst->IndexCount;
					for (uint32_t i = 0; i < idxCount; ++i)
					{
						batchIndices.emplace_back(localIndices[i] + totalVertexCount);
					}

					totalVertexCount += verticesWritten;
					totalIndexCount += idxCount;
				}

				if (totalVertexCount == 0 || totalIndexCount == 0) continue;
				...
				std::shared_ptr<IndexBuffer> batchIB = IndexBuffer::Create(batchIndices.data(), totalIndexCount);
				s_Data.MeshVertexArray->SetIndexBuffer(batchIB);

				key.ShaderPtr->Bind();
				RenderCommand::DrawIndexed(s_Data.MeshVertexArray, totalIndexCount);
				s_Data.Stats.DrawCalls++;
			}
		}
	}
	...
	void Renderer2D::DrawMesh(const glm::vec3& position,
		const glm::vec2& size,
		float rotation,
		const std::shared_ptr<VertexArray>& vertexArray,
		const std::shared_ptr<Shader>& shader,
		uint32_t indexCount,
		const BuildCallback& buildCallback,
		int entityID)
	{
		glm::mat4 transform = glm::translate(glm::mat4(1.0f), position)
			* glm::rotate(glm::mat4(1.0f), glm::radians(rotation), { 0.0f, 0.0f, 1.0f })
		...
		DrawMesh(transform, vertexArray, shader, indexCount, buildCallback, entityID);
	}

	void Renderer2D::DrawMesh(const glm::mat4& transform,
		const std::shared_ptr<VertexArray>& vertexArray,
		const std::shared_ptr<Shader>& shader,
		uint32_t indexCount,
		const BuildCallback& buildCallback,
		int entityID)
	{
		if (!vertexArray || !shader || indexCount == 0 || !buildCallback)
			return;


		auto& inst = s_Data.MeshInstances[s_Data.MeshInstanceCount++];
		inst.Transform = transform;
		inst.VertexArray = vertexArray;
		inst.Shader = shader;
		inst.IndexCount = indexCount;
		inst.BuildVertices = buildCallback;

		s_Data.Stats.MeshIndexCount += indexCount;
	}
